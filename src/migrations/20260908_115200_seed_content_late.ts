...
/* Повторное наполнение: первая seed-миграция выполняется до того, как схема получает поле speakerSlots, поэтому на чистой базе её запрос падает. Эта копия идёт последней и добирает то, что осталось пустым: если на боевой базе глобалы уже существуют,
